...
while (( $# )); do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: $0 --config PATH [--output-dir DIR] [--compile] [--measure-neighbor-padding] [--measure-input] [--precision fp32|fp16|bf16] [--num-workers N] [--max-steps N] [--warmup-steps N] [--dry-run]"
            exit 0 ;;
        *) args+=("$1"); shift ;;
    esac
done
