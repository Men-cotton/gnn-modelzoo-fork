...
dry_run=0
usage() {
    echo "Usage: $0 --output DIR [--dataset arxiv|products] [--backend both|fixed_shape|pyg] [--base-config PATH] [--pyg-base-config PATH] [--workers '2 4 ... 64'] [--phase all|tune|workers|prefetch1|persistent-off|feature-cache] [--compile] [--dry-run]"
}
while (( $# )); do
    case "$1" in
        --base-config|--pyg-base-config|--dataset|--backend|--output|--workers|--phase)
            [[ -n "${2:-}" ]] || { usage >&2; exit 2; }
            case "$1" in
...
for value in "$config" "$pyg_config" "$output"; do
    if [[ "$value" == *','* || "$value" == *$'\n'* ]]; then
        echo 'Paths must not contain commas or newlines.' >&2; exit 2
    fi
done
command=(qsub -v "GPU_SWEEP_DATASET=${dataset},GPU_SWEEP_CONFIG=${config},GPU_SWEEP_OUTPUT=${output},GPU_SWEEP_COMPILE=${compile},GPU_SWEEP_WORKERS=${workers},GPU_SWEEP_PHASE=${phase},GPU_SWEEP_BACKEND=${backend},GPU_SWEEP_PYG_CONFIG=${pyg_config}"
