...
- Fixed-shape CSX batches repeat deterministic target order/neighborhoods; ordinary PyG sampling does not establish the same optimization trajectory. The uncommitted local PyG patch also honors configured precision during evaluation; that runtime change is excluded from this documentation commit. When changing these comparison conditions, remeasure H100 as well.
