...
class PreconditionedConjugateGradientDataProcessorConfig(DataConfig):
    data_processor: Literal["PreconditionedConjugateGradientDataProcessor"]
...
def _run_pcg(x, b, center, iterations, epsilon):
    inv_diagonal = 1.0 / center
...
class PreconditionedConjugateGradientDataProcessor:
    def __init__(self, config: PreconditionedConjugateGradientDataProcessorConfig):
        if isinstance(config, dict):
            config = PreconditionedConjugateGradientDataProcessorConfig(**config)
        self.config = config
    ...
    def create_dataloader(self):
        ...
        reference = _run_pcg(x, b, center, cfg.iterations, cfg.epsilon)
        data = (
