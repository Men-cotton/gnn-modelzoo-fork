...
class GemvCollectives2dDataConfig(DataConfig):
    data_processor: Literal["GemvCollectives2dDataProcessor"] = (
        "GemvCollectives2dDataProcessor"
    )
    batch_size: int = 1
    sample_count: int = 5
    matrix_rows: int = 32
...
class GemvCollectives2dDataProcessor:
    def __init__(self, config: GemvCollectives2dDataConfig):
        if isinstance(config, dict):
            config = GemvCollectives2dDataConfig(**config)
        self.config = config
