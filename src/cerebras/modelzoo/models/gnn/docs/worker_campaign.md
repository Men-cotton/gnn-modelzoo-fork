...
worker感度測定、入力条件を変える比較、詳細診断をまとめて実行し、
最後に結果をZIPへ保存する。学習曲線を取得してから設定を選択する場合は
[学習と探索](learning_campaign.md) を使う。
...
接続方法、ログ、終了コード、csctlのlabelは[起動と監視](worker_launch.md)を参照する。
...
通常runは対象頂点数/秒、教師ラベル有効頂点数/秒、公称枠数/秒を保存する。
公称枠数はパディングを含む。静的バッチ再生は原因調査用の公称枠数/秒として残し、
通常runの対象頂点数/秒と集計を分ける。入力バッチ順の記録と導出条件は
[autotune](autotune.md) を参照する。全体速度と親next時間の差から、
...
失敗があった場合は全予定を消化しても状態を `completed_with_failures`、ドライバの終了コードを2とし、
...
候補名なしの `--controls` は介入を省略する。候補worker数を明示して実行範囲を変更できる。
