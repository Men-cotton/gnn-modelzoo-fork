...
class WorkerSensitivityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.base = tune.load_params_file(
            tune.GNN / "configs/autotune/arxiv_w40.yaml"
        )

    def study(self, args=None):
        return tune.Study(
            args or self.args, self.base, self.output, {"test": "host"}
        )
    ...
    def test_all_conditions_unstable_included_and_resume(self):
        def execute(cmd, log, timeout):
            ...
            self.assertEqual(
                params["trainer"]["init"]["loop"]["max_steps"], 440
            )
    ...
    def test_cpu_preflight_does_not_silently_drop_baseline(self):
        with (
            ...
            with self.assertRaisesRegex(ValueError, "No jobs submitted"):
                tune.main(self.cli)
            self.assertEqual(
                tune.main(self.cli + ["--dry-run", "--cache", "full"]), 0
            )
        ...
        self.assertEqual(
            a["trainer"]["fit"]["train_dataloader"]["cache_fraction"], 1.0
        )
        for config in (a, b):
    ...
    def test_timeout_pause_acknowledgement_and_budget_resume(self):
        ...
        report = json.loads((self.output / "sensitivity.json").read_text())
        self.assertEqual(
            report["status"], "completed_with_missing_measurements"
        )
