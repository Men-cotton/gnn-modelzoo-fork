...
def main(expected_architecture=None, enable_graphsage_options=True):
    ...
    ap.add_argument(
        "--deterministic",
        action="store_true",
        help="Enable strict determinism (slower, but reproducible)",
    )
    ...
    if rank == 0:
        if world_size > 1:
            ...
            print("[Info] Running in Single-Process mode (CPU or Single GPU).")
    ...
    try:
        ...
        if args.use_partitions:
            ...
            from cerebras.modelzoo.models.gnn.tools.partition_dataset import (
                get_partition_path,
            )
            ...
            if data_dir is None:
                print(
                    "[warn] data_dir is missing from dataset_profiles; cannot load partitions.",
                    file=sys.stderr,
                )
                raise SystemExit(1)
        ...
        loaders = make_loaders(
            loader_data, split_idx, cfg, rank=rank, world_size=world_size
        )
